"""Run Section 7 Q25 analytics and optionally generate Gemini insights."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Section 7 pipeline runner for Q25.")
    parser.add_argument("--with-llm", action="store_true", help="Also run Gemini insights generation for Q25.")
    parser.add_argument("--llm-delay", type=int, default=12, help="Delay between files for Gemini generation.")
    parser.add_argument("--llm-model", type=str, default=None, help="Preferred model for Section 7 LLM insights.")
    parser.add_argument(
        "--llm-strict-model",
        action="store_true",
    ...
    steps = [
        (
            [sys.executable, "src/analysis/section7/q25_recruitment_hiring_impact.py"],
            "Q25 analysis",
        ),
    ]
    ...
    if args.with_llm:
        llm_cmd = [
            ...
            "reports/section7/q25_recruitment_hiring_impact.json",
            "--delay",
            str(args.llm_delay),
        ]
        if args.llm_model:
        ...
        steps.append(
            (
                llm_cmd,
                "Section 7 Gemini insights generation",
            )
        )
    ...
    print("[DONE] Section 7 pipeline completed.")
